      ...
      USE globals, ONLY: rp,ne,el_type,order,mndof,elxy, &
      ...
      !  nnd = nnds(et)   ! DSG: possibly an error? added order(et)
        nnd = nnds(order(et)) ! DSG: still messed up?!
        nqa = nqpta(et)
        ndf = ndof(et)
 ...
 !         PRINT*, "Jacobian determinant at area quadrature point, el = ",el
 !         PRINT*, detJa(el,pt)
