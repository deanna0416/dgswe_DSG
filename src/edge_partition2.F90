 ...
 !          PRINT*, elcnt,ael2gel(elcnt)
      ...
      DO pe = 1,npart
        DO ed = 1,nied
          ...
          ged = iedn(ed)   ! iedn is array of interior edge numbers
          el_in = ged2el(1,ged)
          el_ex = ged2el(2,ged)
          ...
          IF (pe_in == pe .and. pe_ex == pe) THEN
            ...
            npartpt(pe) = npartpt(pe) + nqpte(el_type(el_in))
      ...
      DO ed = 1,nied
        IF (edflag(ed) == 0) THEN
          ...
          npartpt(npart+1) = npartpt(npart+1) + nqpte(el_type(el_in))
...
!       rnfblk(2,nrblk) = nied
      ...
      IF (myrank == 0) THEN
        ...
        DO blk = 1,npart+1
          ...
          PRINT*, elblk(1,blk,5), elblk(2,blk,5), npartet(5,blk)
          PRINT*, " "
        ENDDO
        ...
        PRINT*, "nfblk: "
        DO blk = 1,npart+1
          PRINT*, nfblk(1,blk), nfblk(2,blk), nparted(blk)   ! max size ends up being nqpte*nied
        ENDDO
...
!       PRINT "(I8,8x,3(I8),8X,3(I8))", ed,el_in,ael_in,gel2part(el_in), el_ex,ael_ex,gel2part(el_ex)
   !   PRINT "(I8,8x,3(I8),8X,3(I8))", ged,el_in,ael_in,gel2part(el_in), el_ex,ael_ex,gel2part(el_ex)
